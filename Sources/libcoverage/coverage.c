...
void print_cov_context(struct cov_context *context, char *filename)
{
    // 打开文本文件，以追加模式写入
    ...
    // 打印属性到文件
    fprintf(file, "cov_context (id: %d, should_track_edges: %d, num_edges: %u, bitmap_size: %u, found_edges: %u)\n",
            context->id, context->should_track_edges, context->num_edges, context->bitmap_size, context->found_edges);

    // 打印指针地址到文件
    fprintf(file, "virgin_bits: %p, crash_bits: %p, edge_count: %p, shmem: %p\n",
            (void *)context->virgin_bits, (void *)context->crash_bits, (void *)context->edge_count, (void *)context->shmem);
