...
public final class Program {
    ...
    // cjy--store the program's triggered edge
    public var triggeredEdge: Set<UInt32>
 
    // cjy--store the program's triggered edge

    // cjy--创建表示时间差的变量并初始化为0
    public var execTime: TimeInterval
    // cjy--创建表示时间差的变量并初始化为0

    // Public variable to store an integer
    public var addSGed: Int
    ...
    public init() {
        self.code = Code()
        self.parent = nil
        self.triggeredEdge = Set<UInt32>() // 给 triggeredEdge 初始化一个默认值
        self.execTime = 0 // 初始化 timeDifference 为 0
        self.addSGed = 0
    }
    ...
    public init(with code: Code) {
        ...
        self.code = code
        self.triggeredEdge = Set<UInt32>() // 对于第二个构造函数，同样需要对 triggeredEdge 进行初始化
        self.execTime = 0 // 初始化 timeDifference 为 0
        self.addSGed = 0
    }
